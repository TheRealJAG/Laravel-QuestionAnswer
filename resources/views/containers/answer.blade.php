    ...
    <div class="row">
        ...
        <div class="col-xs-10 col-md-11">
            ...
            @if (isset(Auth::user()->id) && $answer->user->id == Auth::user()->id)
                ...
                <div data-type="textarea" data-pk="{{ $answer->id }}" class="answer editable editable-click editable-open editable-custom"  data-title="EDIT"><h4 style="margin: 0;display: inline;">{{ $answer->answer }}</h4></div>
            @else
                <h4 style="margin: 0;display: inline;word-wrap:break-word;">{{ $answer->answer }}</h4>
            @endif

            @if ($answer->created_at != $answer->updated_at)
                <P class="text-right" style="margin-top: 10px"><strong><small><a href="/user/{{$answer->user->id}}" title="{{ $answer->user->name }}">{{ ucfirst($answer->user->name) }}</a> | edited {{ $answer->updated_at->diffForHumans() }}</small></strong></P>
            @else
                <P class="text-right" style="margin-top: 10px"><strong><small><a href="/user/{{$answer->user->id}}" title="{{ $answer->user->name }}">{{ ucfirst($answer->user->name) }}</a> | {{ $answer->created_at->diffForHumans() }}</small></strong></P>
            @endif
